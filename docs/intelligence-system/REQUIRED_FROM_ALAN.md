...
- **Confirm the source registry priorities**: `docs/intelligence-system/SOURCE_REGISTRY.md` now has 34 real, verified sources (P0 multilaterals/DFIs plus the Mexico/Brazil/Chile/Colombia government/regulatory layer), seeded without waiting for a manual list per the 2026-08-11 prompt's own instruction. Recommended next batch is Africa government/regulatory (South Africa, Kenya, Nigeria, Ghana first) before the much larger private-capital/asset-manager layer — flag if a different order is wanted.
